# -*- coding: utf-8 -*-
"""Atomic_Habits_LSTM_Predictor

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1mObsKgNrNHCYush1cNK5uVEsTTMwjRLo
"""
...
athlete at Denison University and named to the ESPN Academic AllAmerica Team—an honor given to just thirty-three players across the
...
If you’re an entrepreneur, your goal might be to build a milliondollar business. Your system is how you test product ideas, hire
...
print(f'length of words len(q){len(q)}')

for i in sent.split('\n'):
  print(token.texts_to_sequences([i])[0])

# now merge all the sentences
input_seq=[]
for i in sent.split('\n'):
  global sent_seq
  sent_seq=token.texts_to_sequences([i])[0]

  for i in range(1,len(sent_seq)):
    input_seq.append(sent_seq[:i+1])
...
# total words=1911
# total sent=6820
# max sentence of each sentence=24

maxlen=[len(i) for i in input_seq]
...
pad_input_seq=pad_sequences(input_seq,maxlen=24,padding='pre')
...
y=to_categorical(y,num_classes=1912)
...
model=Sequential()
model.add(Embedding(1912,100,input_length=23))
model.add(LSTM(150))
model.add(Dense(1912,activation="softmax"))
...
# length of words = 1912
# max len sentence=23
test_text=token.texts_to_sequences([test])[0]
pad_test_text=pad_sequences([test_text],maxlen=23,padding='pre')
predict=model.predict(pad_test_text)
print(np.argmax(predict))

test="You do not rise to the level of your goals"

for i in range(10):
  test_1=token.texts_to_sequences([test])[0]
  test_2=pad_sequences([test_1],maxlen=23,padding='pre')
  test_3=np.argmax(model.predict(test_2))
